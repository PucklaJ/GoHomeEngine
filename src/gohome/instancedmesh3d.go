...
func (this *OpenGLInstancedMesh3D) AddVerticesAssimp(mesh *assimp.Mesh, node *assimp.Node, scene *assimp.Scene, level *Level, directory string, preloaded bool) {

}
func (this *OpenGLInstancedMesh3D) AddVertices(vertices []Mesh3DVertex, indices []uint32) {
...
func (this *OpenGLInstancedMesh3D) calculateOffsets() {
	var i uint32
	var offset uint32 = this.numVertices*MESH3DVERTEX_SIZE + this.numIndices*INDEX_SIZE
...
func (this *OpenGLInstancedMesh3D) Load() {
	...
	var indicesSize uint32 = this.numIndices * INDEX_SIZE
...
func (this *OpenGLInstancedMesh3D) Render() {
	...
	if RenderMgr.currentShader != nil {
		if err := RenderMgr.currentShader.SetUniformMaterial(*this.Material); err != nil {
...
func (this *OpenGLInstancedMesh3D) SetMaterial(mat *Material) {
	this.Material = mat
}
func (this *OpenGLInstancedMesh3D) GetMaterial() *Material {
...
func (this *OpenGLInstancedMesh3D) SetF(index uint32, value []float32) {
	offset := this.getOffset(VALUE_FLOAT, index)
	if offset == 0 {
		return
	}
	...
	for i = 0; i < this.numInstances; i++ {
		gl.BufferSubData(gl.ARRAY_BUFFER, int(offset), int(getSize(VALUE_FLOAT)), gl.Ptr(&value[i]))
...
func (this *OpenGLInstancedMesh3D) SetV2(index uint32, value []mgl32.Vec2) {
	offset := this.getOffset(VALUE_VEC2, index)
	if offset == 0 {
		return
	}
	if uint32(len(value)) < this.numInstances {
	...
	var i uint32
	for i = 0; i < this.numInstances; i++ {
		gl.BufferSubData(gl.ARRAY_BUFFER, int(offset), int(getSize(VALUE_VEC2)), gl.Ptr(&value[i][0]))
		offset += this.instancedSize
	}
...
func (this *OpenGLInstancedMesh3D) SetV3(index uint32, value []mgl32.Vec3) {
	offset := this.getOffset(VALUE_VEC3, index)
	if offset == 0 {
		return
	}
	if uint32(len(value)) < this.numInstances {
	...
	var i uint32
	for i = 0; i < this.numInstances; i++ {
		gl.BufferSubData(gl.ARRAY_BUFFER, int(offset), int(getSize(VALUE_VEC3)), gl.Ptr(&value[i][0]))
		offset += this.instancedSize
	}
...
func (this *OpenGLInstancedMesh3D) SetV4(index uint32, value []mgl32.Vec4) {
	offset := this.getOffset(VALUE_VEC4, index)
	if offset == 0 {
		return
	}
	if uint32(len(value)) < this.numInstances {
	...
	var i uint32
	for i = 0; i < this.numInstances; i++ {
		gl.BufferSubData(gl.ARRAY_BUFFER, int(offset), int(getSize(VALUE_VEC4)), gl.Ptr(&value[i][0]))
		offset += this.instancedSize
	}
...
func (this *OpenGLInstancedMesh3D) SetM2(index uint32, value []mgl32.Mat2) {
	offset := this.getOffset(VALUE_MAT2, index)
	if offset == 0 {
		return
	}
	if uint32(len(value)) < this.numInstances {
	...
	var i uint32
	for i = 0; i < this.numInstances; i++ {
		gl.BufferSubData(gl.ARRAY_BUFFER, int(offset), int(getSize(VALUE_MAT2)), gl.Ptr(&value[i][0]))
		offset += this.instancedSize
	}
...
func (this *OpenGLInstancedMesh3D) SetM3(index uint32, value []mgl32.Mat3) {
	offset := this.getOffset(VALUE_MAT3, index)
	if offset == 0 {
		return
	}
	if uint32(len(value)) < this.numInstances {
	...
	var i uint32
	for i = 0; i < this.numInstances; i++ {
		gl.BufferSubData(gl.ARRAY_BUFFER, int(offset), int(getSize(VALUE_MAT3)), gl.Ptr(&value[i][0]))
		offset += this.instancedSize
	}
...
func (this *OpenGLInstancedMesh3D) SetM4(index uint32, value []mgl32.Mat4) {
	offset := this.getOffset(VALUE_MAT4, index)
	if offset == 0 {
		return
	}
	if uint32(len(value)) < this.numInstances {
	...
	var i uint32
	for i = 0; i < this.numInstances; i++ {
		gl.BufferSubData(gl.ARRAY_BUFFER, int(offset), int(getSize(VALUE_MAT4)), gl.Ptr(&value[i][0]))
		offset += this.instancedSize
	}

	gl.BindBuffer(gl.ARRAY_BUFFER, 0)
}
func (this *OpenGLInstancedMesh3D) GetVertices() []Mesh3DVertex {
