...
func CreateOpenGLES3Mesh2D(name string) *OpenGLES3Mesh2D {
	mesh := OpenGLES3Mesh2D{
		Name: name,
	}
...
func (oglm *OpenGLES3Mesh2D) deleteElements() {
...
func (oglm *OpenGLES3Mesh2D) AddVertices(vertices []gohome.Mesh2DVertex, indices []uint32) {
	oglm.vertices = append(oglm.vertices, vertices...)
	oglm.indices = append(oglm.indices, indices...)
}

func (oglm *OpenGLES3Mesh2D) attributePointer() {
...
func (oglm *OpenGLES3Mesh2D) Load() {
...
func (oglm *OpenGLES3Mesh2D) Render() {
	gl.BindVertexArray(oglm.vao)

	gl.GetError()
	gl.DrawElements(gl.TRIANGLES, int32(oglm.numIndices), gl.UNSIGNED_INT, nil)
	handleOpenGLES3Error("Mesh2D", oglm.Name, "RenderError: ")

	gl.BindVertexArray(0)
}

func (oglm *OpenGLES3Mesh2D) Terminate() {
	var vbuf [1]uint32
	vbuf[0] = oglm.vao
