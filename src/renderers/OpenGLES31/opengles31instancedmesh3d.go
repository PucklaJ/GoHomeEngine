...
func (this *OpenGLES31InstancedMesh3D) getInstancedSize() int {
	var sumSize = 0
...
func (this *OpenGLES31InstancedMesh3D) instancedVertexAttribPointer(verticesSize, indicesSize, sizeOfOneInstance int) {
...
func (this *OpenGLES31InstancedMesh3D) recreateBuffer(numInstances int) {
	verticesSize := this.numVertices * gohome.MESH3DVERTEXSIZE
	indicesSize := this.numIndices * gohome.INDEXSIZE
...
func (this *OpenGLES31InstancedMesh3D) addValueTypeIndexOffset(valueType int) {
	var maxIndex = 0
	for i := 0; i < len(this.valueTypeIndexOffsets); i++ {
...
func (this *OpenGLES31InstancedMesh3D) addValueTypeIndexOffsetFront(valueType int) {
	var maxIndex = 0
	for i := 0; i < len(this.valueTypeIndexOffsets); i++ {
		if this.valueTypeIndexOffsets[i].valueType == valueType {
			if this.valueTypeIndexOffsets[i].index >= maxIndex {
...
func (this *OpenGLES31InstancedMesh3D) getOffset(valueType, index int) int {
	for i := 0; i < len(this.valueTypeIndexOffsets); i++ {
