...
func (ogltex *OpenGLES31Texture) bindingPoint() uint32 {
	return gl.TEXTURE_2D
}

func CreateOpenGLES31Texture(name string) *OpenGLES31Texture {
	tex := &OpenGLES31Texture{
		name: name,
	}
	return tex
}

func printOGLTexture2DError(ogltex *OpenGLES31Texture, data []byte, width, height int) {
	err := gl.GetError()
	if err != gl.NO_ERROR {
...
func (ogltex *OpenGLES31Texture) Load(data []byte, width, height int, shadowMap bool) error {
	ogltex.width = width
	...
	if oglRenderer, ok := gohome.Render.(*OpenGLES31Renderer); ok {
...
func (ogltex *OpenGLES31Texture) LoadFromImage(img image.Image) error {
...
func (ogltex *OpenGLES31Texture) Bind(unit uint32) {
	...
	handleOpenGLES31Error("Texture", ogltex.name, "glActiveTexture in Bind with "+strconv.Itoa(int(unit)))
	gl.BindTexture(ogltex.bindingPoint(), ogltex.oglName)
	handleOpenGLES31Error("Texture", ogltex.name, "glBindTexture in Bind")
}
func (ogltex *OpenGLES31Texture) Unbind(unit uint32) {
	newUnit := toTextureUnit(unit)
	gl.GetError()
	gl.ActiveTexture(newUnit)
	handleOpenGLES31Error("Texture", ogltex.name, "glActiveTexture in Unbind with "+strconv.Itoa(int(unit)))
	gl.BindTexture(ogltex.bindingPoint(), 0)
	handleOpenGLES31Error("Texture", ogltex.name, "glBindTexture in Unbind")
}
...
func (ogltex *OpenGLES31Texture) Terminate() {
	var buf [1]uint32
	buf[0] = ogltex.oglName
	gl.DeleteTextures(1, buf[:])
}

func (ogltex *OpenGLES31Texture) SetFiltering(filtering uint32) {
	gl.BindTexture(ogltex.bindingPoint(), ogltex.oglName)
	var filter int32
...
func (ogltex *OpenGLES31Texture) SetWrapping(wrapping uint32) {
	gl.BindTexture(ogltex.bindingPoint(), ogltex.oglName)
	var wrap int32
...
func (ogltex *OpenGLES31Texture) SetBorderColor(col color.Color) {
	gohome.ErrorMgr.Warning("Texture", ogltex.name, "SetBorderColor does not work in OpenGLES 3.1")
}

func (ogltex *OpenGLES31Texture) SetBorderDepth(depth float32) {
	gohome.ErrorMgr.Warning("Texture", ogltex.name, "SetBorderDepth does not work in OpenGLES 3.1")
...
func (ogltex *OpenGLES31Texture) GetData() (data []byte, width int, height int) {
	...
	gohome.ErrorMgr.Warning("Texture", ogltex.name, "GetData does not work in OpenGLES 3.1")
	return
}
